...
input_folder = "/mnt/c/Users/User/Documents/Indra/geo/CDR/parsing/Voice/OTT/Input"
output_path = "/mnt/c/Users/User/Documents/Indra/geo/CDR/parsing/Voice/OTT/Output"
...
file_list = [f for f in os.listdir(input_folder) if f.endswith(".xlsx") and "Voice_OTT" in f and "clean" not in f]
...
def apply_momt_qualification(df, time_window_seconds=5):
    df = df.copy()
    df["Side_Group_Index"] = df.groupby("Side").cumcount()
    df["Pair_ID"] = None
    df["Qualifier"] = None

    # Temporary columns for logic
    df["W"] = df["Call_Type"]
    df["V"] = df["Call_Status"]
    ...
    for i, row in df.iterrows():
        if i in used_indexes or pd.isna(row["Date Time"]):
            continue
        ...
        opposite_call_type = "MT" if call_type == "MO" else "MO"

        # Candidates with opposite side, same call type, same operator, and within time window
        candidates = df[
            (df["W"] == opposite_call_type) &
            (df["Side"] == opposite_side) &
            (df["Operator"] == current_operator) &
            (df["Date Time"].notna()) &
            (df["Date Time"].between(current_time - time_delta, current_time + time_delta)) &
        ...
        if not candidates.empty:
            ...
            df.at[i, "Qualifier"] = get_qual(row, match_row)
            df.at[match_idx, "Qualifier"] = get_qual(match_row, row)
...
def export_country(country_group):
    ...
    output_file = os.path.join(output_path, f"Voice_OTT_{country_name}_clean.xlsx")
