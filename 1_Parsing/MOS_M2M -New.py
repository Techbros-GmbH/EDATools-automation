...
input_folder = "/mnt/c/Users/User/Documents/Indra/geo/CDR/parsing/MOS/M2M/Input"
output_path = "/mnt/c/Users/User/Documents/Indra/geo/CDR/parsing/MOS/M2M/Output"
...
file_list = [f for f in os.listdir(input_folder) if f.endswith(".xlsx") and "MOS_M2M" in f and "clean" not in f]
with ThreadPoolExecutor(max_workers=4) as executor:
    df_list = list(executor.map(load_file, file_list))
df_list = [df for df in df_list if df is not None]
if not df_list:
    raise ValueError("No MOSM2M raw files loaded.")
...
agg_df['Sequence'] = agg_df.apply(
    lambda row: re.search(r"(Voice_\d+)", str(row['VoiceLogfileName_filled'])).group(1)
    if row.get('Country') in ['Austria', 'Bremen'] and re.search(r"(Voice_\d+)", str(row['VoiceLogfileName_filled'])) else None,
    axis=1
)
...
def export_country(country_group):
    ...
    output_file = os.path.join(output_path, f"MOS_M2M_{country_name}_clean.xlsx")
