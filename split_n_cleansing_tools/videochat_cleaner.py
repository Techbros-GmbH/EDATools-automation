...
class VideoChatPackager:
    """
    Transform raw Video Chat XLSX files into a single XLSX output (AllData sheet).
    Returns: Dict[filename -> BytesIO]
    """

    def __init__(self, default_group_key: str = "VideoChat"):
    ...
    def process(self, files: Iterable[Tuple[object, str]]) -> Dict[str, BytesIO]:
        ...
        if not frames:
            raise ValueError("No Video Chat inputs loaded.")
        ...
        df = rename_columns(df, VIDEOCHAT_RENAME_MAP)
        ...
        group_key = VIDEOCHAT_TEST_NAME_TO_GROUP.get(tname, self.default_group_key)

        wanted = list(VIDEOCHAT_SCHEMA_GROUPS.get(group_key, [])) + ["Test_IDs"]
        ...
        out_name = f"{date_str}_{country}_BM_CDRData_VideoChat.xlsx"
...
if __name__ == "__main__":
    ...
    input_dir = Path("/mnt/c/Users/User/Documents/Indra/geo/CDR/parsing/split/Video_Chat/Input")
